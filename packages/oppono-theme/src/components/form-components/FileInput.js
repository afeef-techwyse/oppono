...
const UploadedFileState = styled(({className, file, name, removeFile = () => {}}) => {
  ...
  React.useEffect(() => {
    const tween = gsap.to({x: 0}, {x: 100, onUpdate() {setProgress(this.progress());}, onComplete() {setSuccess(true);}, duration: 30});
  ...
  return (
    <div className={classnames(className, {success, error})}
      ...
      <input type="hidden" name={'appraisal_report_filename'} required value={file.name}/>
