...
`;
const FormFilter = React.forwardRef(({className, children, initial = '*', filters = {'*': 'All'}, type = 'rect', showFilter = true}, forwardedRef) => {
  ...
  React.useEffect(() => {
    gsap.to(combinedRef.current.querySelectorAll(`[data-filter]`), {
      height: (_, target) => current === '*' || target.dataset.filter === '*' || target.dataset.filter === current
        ? 'auto'
        : 0,
    });
  }, [current]);
      ...
      {showFilter
        ...
        >
          {Object.keys(filters).map((rule, filterIndex) =>
            <SwiperSlide key={filterIndex} className={classnames('filter', {active: current === rule})} onClick={() => setCurrent(rule)}>
              {
                type === 'rect'
                  ? <RadioInput label={filters[rule]} name={''} readOnly checked={current === rule}/>
                  : <Button label={filters[rule]} className={classnames('small', {bordered: current !== rule})}/>
              }
