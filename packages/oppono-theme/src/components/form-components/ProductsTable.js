...
const ProductsTable = ({ className, children, dataFilter, products }) => {
  ...
  React.useEffect(() => {
    ...
    elements.current.totalPages?.innerText &&
      (elements.current.totalPages.innerText = tablePages.current);
  }, [media, products]);
  ...
  React.useEffect(() => {
    if (elements.current.table) {
      if (media !== "mobile") {
            ...
            gsap
              ...
              .set(
                elements.current.columns
                  .map((column, columnIndex) =>
                    elements.current.table.querySelectorAll(
                      `tr > :nth-of-type(${columnIndex + 1})`
                    )
                  )
                  .slice(1)
                  .flat(),
              ...
              .set(
                elements.current.columns
                  ...
                  .slice(1)
                  .slice(
                    currentTablePage * numberPerPage,
                    numberPerPage * (currentTablePage + 1)
                  )
                  .flat(),
                { display: "table-cell" }
              ),
        ...
        elements.current.prevArrow?.classList.toggle(
          "disabled",
          currentTablePage <= 0
        );
        elements.current.nextArrow?.classList.toggle(
          "disabled",
          currentTablePage >= tablePages.current - 1
        );
        elements.current.currentPage?.innerText &&
          (elements.current.currentPage.innerText = currentTablePage + 1);
      } else {
        gsap.set(
          elements.current.columns
            .map((column, columnIndex) =>
