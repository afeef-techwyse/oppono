...
const ProductsMobileOption = ({className, children}) => {
  ...
  React.useEffect(() => {
    const showMoreBtn = mobileOptionRef.current.querySelector('.show-all-specs');
    const toggleShowRemaining = () => setShowRemaining(prevState => !prevState);
    showMoreBtn?.addEventListener('click', toggleShowRemaining);
    return () => showMoreBtn?.removeEventListener('click', toggleShowRemaining);
  }, []);
  
  React.useEffect(() => {
    gsap.to(mobileOptionRef.current.querySelector('.remaining-specs'), {height: showRemaining ? 'auto' : 0});
    mobileOptionRef.current.querySelector('.show-all-specs')?.classList.toggle('opened', showRemaining);
  }, [showRemaining]);
  return <div ref={mobileOptionRef} className={className} children={children}/>;
};
