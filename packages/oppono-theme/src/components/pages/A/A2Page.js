  ...
  return <div className={className}>
    <Form endPoint={'/purchase'}>
      ...
      <FormStep apiStepNumber={2} pageName={pageName} activeTheme={formData.section_2?.section_theme} stepName={formData.section_2?.section_name}>
        ...
        <RadioGroup radioText={formData.section_2?.looking_for_yes_no.label} checked={'first'}>
          <RadioInput label={formData.section_2?.looking_for_yes_no.yes} value={'first'} name={'looking_for'} type={'radio'}/>
          <RadioInput label={formData.section_2?.looking_for_yes_no.no} value={'second'} name={'looking_for'} type={'radio'}/>
        </RadioGroup>
  
        <W50>
          <Input type={'text'} pattern={'[0-9]+'} name={'purchase_price'} {...formData.section_2?.purchase_price_input}/>
      ...
      <FormStep endPoint={null} pageName={pageName}
                activeTheme={formData.section_4?.section_theme} stepName={formData.section_4?.section_name}>
        ...
        <Finalize>
          ...
          <Bottom>
            ...
            <FinalizeChild order={3} className={'m-pr-40 full m-border'}>
              ...
              <P.Border>Your property value is ${+section2Values('purchase_price')}</P.Border>
            </FinalizeChild>
            <FinalizeChild order={3} className={'wide m-pr-40'}>
              <P.Border>Your down payment is ${+section2Values('down_payment')}</P.Border>
      ...
      <FormStep apiStepNumber={4} pageName={pageName} activeTheme={formData.section_5?.section_theme} stepName={formData.section_5?.section_name}>
        <div className="form-text-wrapper wide-text">
          ...
          <h1 className={'form-headline-1 text-left'}>{formData.section_5?.title.replace('{{number}}', state.theme.stepResponse.data?.data?.[section2Values('looking_for')]?.products.length)}</h1>
          ...
          ? state.theme.stepResponse.data?.data?.[section2Values('looking_for')]?.products.map((product, index) => <Finalize key={product.ID}>
            <Top>
              ...
              <FinalizeChild className={'wide'} order={1}>
                ...
                <P.Cost>${monthlyPayments(mortgage, product.fields?.rate)}</P.Cost>
              </FinalizeChild>
            </Top>
            <Bottom>
                ...
                ? <FinalizeChild order={2} className={'full m-border'}>
                  <FinalizeTable>
                    ...
                    <tr>
                      ...
                      <P.D as={'td'}>Up to {product.fields?.maximum_ltv}%</P.D>
                    </tr>
                    <tr>
              ...
              <FinalizeChild order={3} className={'wide m-pr-40'}>
                {product.fields?.specifications.map(({term_id, name}) => <P.Border key={term_id}>{name}</P.Border>)}
              </FinalizeChild>
            </Bottom>
          </Finalize>) : null}
      </FormStep>
