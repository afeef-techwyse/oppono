...
const C1Page = ({className, setCurrentTheme, state, actions, formData}) => {
  ...
  const mortgage = ((+section5Values('mortgage_value_1') || 0) + (+section5Values('mortgage_value_2') || 0) + (+section5Values('outstanding_amount_value')) + (+section5Values('sm_amount')) + (+section5Values('fm_amount')) || 0) || 0;
  ...
  return <div className={className}>
    <Form setCurrentTheme={setCurrentTheme} endPoint={'/refinance'}>
              ...
              {Object.entries(state.theme.stepResponse.data?.data).map(([type, {products}], index) =>
                <ProductsTable products={productsTable} key={type} dataFilter={type}>
                  ...
                  <tr className={'head last-head'}>
                    ...
                    {products.map(({ID, fields: {beacon_score}}) => <td key={ID} className={'details'} data-label="beacon_score">{beaconScore(beacon_score)}</td>)}
                  </tr>
      ...
      <FormStep apiStepNumber={2} pageName={pageName} activeTheme={formData.section_5?.section_theme} stepName={formData.section_5?.section_name}>
        ...
        <Input type={'number'} name={'home_value'} {...formData.section_5?.estimated_value_input} value={section1Values('home_value')}/>
      ...
      <FormStep apiStepNumber={4} pageName={pageName} activeTheme={formData.section_7?.section_theme} stepName={formData.section_7?.section_name}>
        ...
        <input type={'hidden'} name={`maximun_mortgage`} value={Math.round(+section1Values('home_value') * firstProduct.fields?.maximum_ltv / 100)}/>
      ...
      <FormStep apiStepNumber={5} pageName={pageName} activeTheme={formData.section_8?.section_theme} stepName={formData.section_8?.section_name}>
        <div className="upload-step-wrapper">
          ...
          <FormConditionalInput noScroll name={'mortgages_1'} showOn={'1'} checked={'0'} {...formData.section_8?.have_appraisal_report_yes_no}>
